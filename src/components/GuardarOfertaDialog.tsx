...
export function GuardarOfertaDialog({ oferta }: { oferta: Oferta }) {
  ...
  async function guardar() {
    ...
    try {
      ...
      toast.success("Voo guardado em Minhas viagens.");
      setAberto(false);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Não foi possível guardar o voo.");
    } finally {
      setAguardar(false);
    }
  }
  ...
  return (
    <Dialog open={aberto} onOpenChange={setAberto}>
      <DialogTrigger asChild>
        <Button size="sm">
          <BookmarkPlus className="size-4" /> Guardar
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
